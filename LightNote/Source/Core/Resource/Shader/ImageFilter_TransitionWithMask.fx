    ...
    float2  in_uv_   : TEXCOORD0 ) : COLOR
{
    ...
    float t = tex2D( gTransitionMaskSampler, in_uv_ ).r;

    // 移り変わる前のイメージのアルファ値
    t = ( t - gTransitionTime ) / gTransitionVague;

    // 移り変わる前と後を補間して完了
    out_color = lerp( out_color, tex2D( gTransitionFreezedSampler, in_uv_ ), clamp( t, 0.0, 1.0 ) );
