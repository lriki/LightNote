...
//==============================================================================
	...
	//
	//----------------------------------------------------------------------
	ALOnMemoryPlayer::ALOnMemoryPlayer( ALAudioDevice* device )
	...
	//
	//----------------------------------------------------------------------
	ALOnMemoryPlayer::~ALOnMemoryPlayer()
	{
		stop();
	}
	...
	//
	//----------------------------------------------------------------------
	void ALOnMemoryPlayer::initialize( AudioSourceBase* audioSource, bool enable3d )
	{
		AudioPlayerBase::initialize( audioSource, enable3d );
	...
	//
	//----------------------------------------------------------------------
	void ALOnMemoryPlayer::setVolume( int volume )
	{
	}
	...
	//
	//----------------------------------------------------------------------
	void ALOnMemoryPlayer::setPitch( int pitch )
	{
	}
	...
	//
	//----------------------------------------------------------------------
    lnU64 ALOnMemoryPlayer::getPlayedSamples() const
	{
		return 0;
	}
	...
	//
	//----------------------------------------------------------------------
	void ALOnMemoryPlayer::play()
	{
		const WaveFormat* format = mAudioSource->getWaveFormat();
		LN_ERR2_ASSERT( format->Channels == 1 || format->Channels == 2 );
		LN_ERR2_ASSERT( format->BitsPerSample == 8 || format->BitsPerSample == 16 );
		
		ALenum alChannelFormat;
		...
		// バッファ作成 (Source と Buffer は1対1)
		ALLib::alGenBuffers( 1, &mALBuffer );
		ALLib::alBufferData( 
			mALBuffer, 
			alChannelFormat, 
		...
		// ソースを作ってバッファと関連付ける
		ALLib::alGenSources( 1, &mALSource );
		ALLib::alSourcei( mALSource, AL_BUFFER, mALBuffer );

		// 再生
		ALLib::alSourcePlay( mALSource );
	...
	//
	//----------------------------------------------------------------------
	void ALOnMemoryPlayer::stop()
	{
		// 再生を止めてソースを開放
		if ( mALSource ) 
		{
			ALLib::alSourceStop( mALSource );
			ALLib::alDeleteSources( 1, &mALSource );
			mALSource = 0;
		}
		...
		if ( mALBuffer ) 
		{
			ALLib::alDeleteBuffers( 1, &mALBuffer );
			mALBuffer = 0;
		}
	}
	...
	//
	//----------------------------------------------------------------------
	void ALOnMemoryPlayer::pause( bool isPause )
	{
	}
	...
	//
	//----------------------------------------------------------------------
	bool ALOnMemoryPlayer::polling()
	{
		// 停止判定
		ALint state;
		ALLib::alGetSourcei( mALSource, AL_SOURCE_STATE, &state );
        if ( state != AL_PLAYING ) {
            return false;
        }
