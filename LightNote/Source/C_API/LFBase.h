...
LN_STRUCT_CLASS(LNRect)
	...
	*/
	...
	/**
		@brief		指定した x y 座標の点が矩形に含まれているかを判別します。
		@param[in]	rect	: 処理の基になる Rect
		@param[in]	x		: X 座標
		@param[in]	y		: Y 座標
...
LN_EXTENSION_CLASS(LNException)
	...
	*/
	...
	/**
		@brief		最後に発生した例外のエラーコードを取得します。
	*/
	LN_STATIC_API
	LNResult LNException_GetLastErrorCode();

	/**
		@brief		最後に発生した例外に対して、デフォルトの処理を行います。(メッセージボックスを表示し、LNException_SetHandled(true))
	*/
	LN_STATIC_API
...
LN_CLASS(LNIntTable)
	...
	*/
	...
	/**
		@brief		整数型テーブルの指定したインデックスに値を設定します。
